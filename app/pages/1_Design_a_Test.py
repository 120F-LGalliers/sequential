...
if "design_result" in st.session_state:
    ...
    if total_days is not None:
        look_labels = [f"{k} (Day {round(t * total_days)})" for k, t in zip(range(1, inp.n_looks + 1), result.t)]
    else:
        look_labels = list(range(1, inp.n_looks + 1))
    table = {
        "Look": look_labels,
    ...
    if total_days is not None:
        st.caption(
            "Day numbers in brackets are projected calendar days from launch, based on the expected "
            "weekly traffic entered above -- a planning estimate, not a fixed schedule. Actual looks "
            "happen whenever you check in and are read at whatever information fraction has actually "
            "been reached by then, not the day shown here."
        )
